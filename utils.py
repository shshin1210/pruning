...
class Cutout(object):
    ...
    def __call__(self, img):
        ...
        for n in range(self.n_holes): # 0
            y = np.random.randint(h)
            x = np.random.randint(w)

            y1 = np.clip(y - self.length // 2, 0, h) # 0 보다 작은값은 0, h보다 큰 값은 h
            y2 = np.clip(y + self.length // 2, 0, h)
            x1 = np.clip(x - self.length // 2, 0, w)
            x2 = np.clip(x + self.length // 2, 0, w)

            mask[y1:y2, x1:x2] = 0.0 # y1부터 y2 까지, x1부터 x2까지 0 대입 (나머지 1)

        mask = torch.from_numpy(mask)
        mask = mask.expand_as(img) # expand tensor size as img
        img = img * mask 
